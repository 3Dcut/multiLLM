...
Sub CreateSetupScript()
    ...
    File.WriteLine "    $hta = Join-Path $AppPath 'assets\status.hta'"
    File.WriteLine "    if (Test-Path $hta) { Start-Process mshta $hta }"
    File.WriteLine "    Start-Sleep -Milliseconds 500"
    File.WriteLine "}"
    File.WriteLine ""
    File.WriteLine "function Show-Disclaimer {"
    File.WriteLine "    $hta = Join-Path $AppPath 'assets\disclaimer.hta'"
    ...
    File.WriteLine "    if (-not (Test-Path (Join-Path $AppPath 'src\main\main.js'))) {"
    ...
    File.WriteLine "$isFirstInstall = -not (Test-Path (Join-Path $AppPath 'src\main\main.js'))"
    File.WriteLine "$needsUpdate = $remote.date -gt $localTs"
    File.WriteLine ""
    File.WriteLine "# Download disclaimer if needed"
    File.WriteLine "if ($isFirstInstall -or ($needsUpdate -and -not (Test-Path $DisclaimerAccepted))) {"
    File.WriteLine "    Download-File ""$GitHubRaw/assets/disclaimer.hta"" (Join-Path $AppPath 'assets\disclaimer.hta')"
    ...
    File.WriteLine "             'src/ui/index.html'='index.html';'src/ui/styles.css'='styles.css'}"
    File.WriteLine "$assetsFiles = @{'assets/status.hta'='status.hta';'assets/disclaimer.hta'='disclaimer.hta';"
    File.WriteLine "                'assets/Uninstall.hta'='Uninstall.hta'}"
    File.WriteLine "$configFiles = @{'config/config.json.template'='config.json.template';"
    File.WriteLine "                'config/user-settings.json.template'='user-settings.json.template'}"
    File.WriteLine ""
    File.WriteLine "# First install"
    File.WriteLine "if ($isFirstInstall) {"
    File.WriteLine "    Download-File ""$GitHubRaw/assets/status.hta"" (Join-Path $AppPath 'assets\status.hta')"
    ...
    File.WriteLine "        Show-Status ""$($t.downloading) $f..."""
    File.WriteLine "        Download-File ""$GitHubRaw/src/$f"" (Join-Path $AppPath $key)"
    File.WriteLine "    }"
    File.WriteLine "    "
    ...
    File.WriteLine "        Show-Status ""$($t.downloading) $f..."""
    File.WriteLine "        Download-File ""$GitHubRaw/assets/$f"" (Join-Path $AppPath $key)"
    File.WriteLine "    }"
    File.WriteLine "    "
    ...
    File.WriteLine "        Show-Status ""$($t.downloading) $f..."""
    File.WriteLine "        Download-File ""$GitHubRaw/config/$f"" (Join-Path $AppPath $key)"
    File.WriteLine "    }"
    File.WriteLine "    "
    File.WriteLine "    # Initialize configs"
    File.WriteLine "    $cfgTemplate = Join-Path $AppPath 'config\config.json.template'"
    File.WriteLine "    $cfg = Join-Path $AppPath 'config\config.json'"
    File.WriteLine "    if ((Test-Path $cfgTemplate) -and -not (Test-Path $cfg)) { Copy-Item $cfgTemplate $cfg }"
    File.WriteLine "    "
    File.WriteLine "    $usTemplate = Join-Path $AppPath 'config\user-settings.json.template'"
    File.WriteLine "    $us = Join-Path $AppPath 'config\user-settings.json'"
    ...
    File.WriteLine "    "
    File.WriteLine "    # Offer desktop shortcut"
    File.WriteLine "    Close-Status"
    ...
    File.WriteLine "    $msg = ""$($t.updateAvailable)`n`n$($t.newVersion): $($remote.date)`n$($t.currentVersion): $localTs`n`n$($t.updateNow)"""
    File.WriteLine "    $r = [System.Windows.Forms.MessageBox]::Show($msg, 'LLM MultiChat', 'YesNo', 'Question')"
    ...
    File.WriteLine "            Close-Status"
    File.WriteLine "            $r = [System.Windows.Forms.MessageBox]::Show($t.updateModules, 'LLM MultiChat', 'YesNo', 'Question')"
    File.WriteLine "            if ($r -eq 'Yes') {"
    File.WriteLine "                Start-StatusWindow"
    File.WriteLine "                Show-Status $t.deletingModules"
    ...
    File.WriteLine "    $htaExists = Test-Path (Join-Path $AppPath 'assets\status.hta')"
